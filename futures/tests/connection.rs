    ...
    core.run(
        TcpStream::connect(&addr, &handle).and_then(|stream| {
            ...
            client.create_channel().and_then(|consume_channel| {
                ...
                consume_channel.queue_declare("hello_not_ready_polls_again", &QueueDeclareOptions::default(), &FieldTable::new()).and_then(move |_| {
                    // Purge the queue so we don't end up with messages from previous test runs!
                    consume_channel.queue_purge("hello_not_ready_polls_again", &QueuePurgeOptions::default()).map(move |_| {
                        println!("channel {} declared queue {}", id, "hello_not_ready_polls_again");

                        handle.spawn(
                            consume_channel.basic_consume("hello_not_ready_polls_again", "my_consumer", &BasicConsumeOptions::default(), &FieldTable::new()).and_then(|stream| {
                    ...
                    client.create_channel().and_then(move |publish_channel| {
                        ...
                        Timeout::new(Duration::from_secs(1), &handle_for_timeout).unwrap().and_then(move |_| {
                    ...
                    futures::future::empty::<(), std::io::Error>()
                })
            })
        })
    ).unwrap();
